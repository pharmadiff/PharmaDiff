...
from pharmadiff.datasets.pharmacophore_utils import prepare_pharma_data
...
class NoiseModel:
    ...
    def move_P_device(self, tensor):
        ...
        return diffusion_utils.PlaceHolder(X=self.Px.float().to(tensor.device),
                                           ...
                                           pharma_coord=None, pharma_feat=None, pharma_atom=None)
    ...
    def apply_noise(self, dense_data, t_int=None):
        ...
        noise_pos_masked, _ = utils.remove_mean_with_mask(noise_pos_masked, dense_data.node_mask)
    ...
    def get_limit_dist(self):
        ...
        limit_dist = utils.PlaceHolder(X=X_marginals, E=E_marginals, charges=charges_marginals,
                                       y=None, pos=None, pharma_coord=None, pharma_feat=None)
        return limit_dist

    def sample_limit_dist(self,data, node_mask, sample_condition, n_nodes):
        ...
        if sample_condition is None:
            ...
            pharma_atom = data.pharma_atom[random_indices]
            ...
            pharma_feat = sample_condition.pharma_feat.repeat(bs, 1, 1)
            pharma_mask = sample_condition.pharma_mask.repeat(bs, 1)
            pharma_atom = sample_condition.pharma_atom.repeat(bs, 1, 1)
            ...
            #pharma_coord, pharma_feat, pharma_mask, pharma_atom = prepare_pharma_data(sample_condition, n_nodes, bs)

        n = pharma_coord.shape[1]
        n_max = max(n_max, n)
        
        
        if n_max > node_mask.shape[1]:
            node_mask = F.pad(node_mask, (0, n_max - node_mask.shape[1]), mode='constant', value=0)

        if n_max > n:
            padding = (0, 0, 0, n_max - n)
            pharma_mask = F.pad(pharma_mask, (0, n_max - n), mode='constant', value=0)
            pharma_coord = F.pad(pharma_coord, padding, mode='constant', value=0) 
            pharma_feat = F.pad(pharma_feat, padding, mode='constant', value=0) 
            pharma_atom = F.pad(pharma_atom, padding, mode='constant', value=0)
            pharma_charge = F.pad(pharma_charge, padding, mode='constant', value=0)
            pharma_atom_pos = F.pad(pharma_atom_pos, padding, mode='constant', value=0)
            pharma_E = F.pad(pharma_E, (0, 0, 0, n_max - n, 0, n_max-n), mode='constant', value=0)
        
        node_mask = node_mask | pharma_mask
        ...
        pos, _ = utils.remove_mean_with_mask(pos, node_mask)
    ...
    def sample_zs_from_zt_and_pred(self, z_t, pred, s_int):
        ...
        noise, _ = utils.remove_mean_with_mask(sampled_pos, node_mask=node_mask)
    ...
    def apply_noise_at_sampling(self, t_int, pos, x, pharma_mask):
        ...
        noise_pos = torch.randn(pos.shape, device=pos.device)
        noise_pos_masked = noise_pos * pharma_mask.unsqueeze(-1)
        noise_pos_masked, _ = utils.remove_mean_with_mask(noise_pos_masked, pharma_mask)

        # beta_t and alpha_s_bar are used for denoising/loss computation
        a = self.get_alpha_bar(t_int=t_int, key='p').unsqueeze(-1)
        s = self.get_sigma_bar(t_int=t_int, key='p').unsqueeze(-1)
        pos_t = a * pos + s * noise_pos_masked
        
        
        noise_x = torch.randn(x.shape, device=x.device)
        noise_x_masked = noise_x * pharma_mask.unsqueeze(-1)
        
        
        # beta_t and alpha_s_bar are used for denoising/loss computation
        a = self.get_alpha_bar(t_int=t_int, key='x').unsqueeze(-1)
        s = self.get_sigma_bar(t_int=t_int, key='x').unsqueeze(-1)
        x_t = a * x + s * noise_x_masked
