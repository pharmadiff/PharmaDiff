...
from pharmadiff.metrics.abstract_metrics import CrossEntropyMetric
...
class TrainLoss(nn.Module):
    ...
    def __init__(self, lambda_train):
        ...
        self.train_pharma_pos = MeanSquaredError(sync_on_compute=False, dist_sync_on_step=False)
        self.train_pharma_X = CrossEntropyMetric()
        self.train_pharma_charges = CrossEntropyMetric()
    ...
    def forward(self, masked_pred, masked_true, log: bool):
        ...
        pharma_mask = masked_true.pharma_mask
        ...
        true_pharma_pos = masked_true.pos[pharma_mask]       # q x 3
        masked_pred_pharma_pos = masked_pred.pos[pharma_mask]        # q x 3
        
        
        true_pharma_X = masked_true.X[pharma_mask]       # q x 4
        masked_pred_pharma_X = masked_pred.X[pharma_mask]        # q x 4
        
        true_pharma_charges = masked_true.charges[pharma_mask]       # q x 3
        masked_pred_pharma_charges = masked_pred.charges[pharma_mask]        # q x 3
        ...
        loss_pharma_pos = self.train_pharma_pos(masked_pred_pharma_pos, true_pharma_pos) if true_pharma_pos.numel() > 0 else 0.0
        loss_pharma_X = self.train_pharma_X(masked_pred_pharma_X, true_pharma_X) if true_pharma_X.numel() > 0 else 0.0
        loss_pharma_charges = self.train_pharma_charges(masked_pred_pharma_charges, true_pharma_charges) if true_pharma_charges.numel() > 0 else 0.0
        ...
        batch_loss = (self.lambda_train[0] * loss_pos + self.lambda_train[1] * loss_X +
                      ...
                      self.lambda_train[4] * loss_y + self.lambda_train[5] * loss_pharma_pos +
                      self.lambda_train[6] * loss_pharma_X + self.lambda_train[7] * loss_pharma_charges) 
        ...
        to_log = {"train_loss/pos_mse": self.lambda_train[0] * self.train_pos_mse.compute() if true_X.numel() > 0 else -1,
                  ...
                  "train_loss/pharma_pos_mse": self.lambda_train[5] * self.train_pharma_pos.compute() if true_pharma_pos.numel() > 0 else -1,
                  "train_loss/pharma_X_CE": self.lambda_train[6] * self.train_pharma_X.compute() if true_pharma_X.numel() > 0 else -1,
                  "train_loss/pharma_charges_CE": self.lambda_train[7] * self.train_pharma_charges.compute() if true_pharma_charges.numel() > 0 else -1,
    ...
    def reset(self):
        for metric in [self.train_pos_mse, self.node_loss, self.charges_loss, self.edge_loss, self.y_loss, 
                       self.train_pharma_pos, self.train_pharma_X, self.train_pharma_charges]:
            metric.reset()
    ...
    def log_epoch_metrics(self):
        ...
        to_log = {"train_epoch/pos_mse": epoch_pos_loss,
                  ...
                  "train_epoch/pharma_pos_mse": epoch_pharma_pos_loss,
                  "train_epoch/pharma_X_CE": epoch_pharma_X_loss,
                  "train_epoch/pharma_charges_CE": epoch_pharma_charges_loss}
